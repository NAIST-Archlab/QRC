...
void H_Gate(int n){
    ...
    // printf("Applying H_Gate with n= %d\n", n);
...
void CNOT_Gate(int control, int target){
    ...
    // printf("Applying CNOT_Gate with control= %d, target = %d\n", control, target);
...
void S_Gate(int n){
    U64 cut = (U64)pow(2, (qubit_num - n - 1));
    // printf("Applying S_Gate with n = %d\n", n);
...
void RX_Gate(int n, float phi){
    U64 cut = (U64)pow(2, (qubit_num - n - 1));
    // printf("Applying RX_Gate with n = %d, phi = %f\n", n, phi);
    *(CGRA_info.reg_mmap + QCTR_N_CUT_MEM_BASE_PHYS + addr) = ((U64)n << 17) | cut;
    ...
    uint32_t sin_n_half = float_to_hex(sin_phi);
    uint32_t cos_n_half = float_to_hex(cos_phi);
...
void RZ_Gate(int n, float phi){
    U64 cut = (U64)pow(2, (qubit_num - n - 1));
    // printf("Applying RZ_Gate with n = %d, phi = %f\n", n, phi);
    *(CGRA_info.reg_mmap + QCTR_N_CUT_MEM_BASE_PHYS + addr) = ((U64)n << 17) | cut;
	    // Calculate sine and cosine
    float sin_phi = sin(phi/2);
    float cos_phi = cos(phi/2);
...
void RY_Gate(int n, float phi){
    U64 cut = (U64)pow(2, (qubit_num - n - 1));
    // printf("Applying RY_Gate with n = %d, phi = %f\n", n, phi);
    *(CGRA_info.reg_mmap + QCTR_N_CUT_MEM_BASE_PHYS + addr) = ((U64)n << 17) | cut;
	    // Calculate sine and cosine
    float sin_phi = sin(phi/2);
    float cos_phi = cos(phi/2);
	...
	unsigned long long sum_time_spent_execution;
		...
		for(int depth = 1; depth < 11; depth++){
			// Create directory structure
				...
				char filename[100];
				sprintf(filename, "time/FP32_%d_Time.txt", depth );
